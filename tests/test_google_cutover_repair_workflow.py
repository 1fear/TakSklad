...
class GoogleCutoverRepairWorkflowTests(unittest.TestCase):
    ...
    def test_repair_workflow_is_exact_scope_backup_first_and_fail_closed(self):
        ...
        self.assertIn('payload.get("prerequisite_return_inserts") != 0', workflow)
        self.assertIn('payload.get("reconstructed_prerequisite_occurrences") != 0', workflow)
        self.assertIn('payload.get("reused_existing_prerequisite_return_occurrences") != 8', workflow)
        self.assertIn('payload.get("reconstructed_missing_scan_boundary_occurrences") != 8', workflow)
