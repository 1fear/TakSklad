...
class ReleasePreflightTests(unittest.TestCase):
                ...
                json.dumps(
                    {
                        ...
                        "min_supported_version": "2.0.13",
                        "mandatory": True,
                        "package_type": "onefile_exe",
                        "download_url": "https://github.com/1fear/TakSklad/releases/download/v2.0.13/TakSklad.exe",
                        "sha256": "a" * 64,
                        "download_url_onedir": "https://github.com/1fear/TakSklad/releases/download/v2.0.13/TakSklad-windows-x64.zip",
                        "sha256_onedir": "b" * 64,
                    },
                    ensure_ascii=False,
                ),
    ...
    def test_version_json_rejects_bad_url_and_sha_format(self):
        ...
        self.assertIn("download_url must be an HTTPS release URL for v2.0.13", check["problems"])
        self.assertIn("download_url_onedir must be an HTTPS release URL for v2.0.13", check["problems"])
    ...
    def test_version_json_rejects_non_forced_or_incomplete_rollout_manifest(self):
        ...
        self.assertFalse(check["ok"])
        self.assertIn("min_supported_version must be 2.0.13 for forced rollout", check["problems"])
