...
class BackendApiPersistenceTests(unittest.TestCase):
    ...
    def legacy_admin_table_counts_pending_google_exports_from_order_ids(self):
    ...
    def test_bulk_complete_without_kiz_marks_unscanned_orders_completed_without_projection_events(self):
        first_order_id, _first_item_id = self.seed_order(quantity_blocks=3)
    ...
    def test_cancel_order_requires_no_scans_without_projection_event(self):
    ...
    def test_delete_active_order_removes_unscanned_order_without_projection_event(self):
    ...
    def test_delete_active_order_idempotency_prevents_duplicate_audit_and_export(self):
        ...
        with self.SessionLocal() as db:
            ...
            self.assertEqual(
                len(db.execute(select(PendingEvent)).scalars().all()),
                0,
            )
    ...
    def legacy_resync_google_for_active_order_pushes_items_without_archiving_order(self):
    ...
    def test_sync_sources_runs_skladbot_sync(self):
        with mock.patch("backend.app.main.update_orders_from_skladbot") as skladbot_sync:
        ...
        self.assertNotIn("google_sheets", payload)
    ...
    def test_sync_sources_starts_skladbot_in_background_by_default(self):
        with mock.patch("backend.app.main.start_skladbot_sync_background") as start_skladbot:
    ...
    def legacy_scan_undo_removes_backend_scan_and_queues_google_projection(self):
    ...
    def legacy_sync_sources_flushes_pending_google_exports_before_google_refresh(self):
        _, item_id = self.seed_order()
        call_order = []
        with self.SessionLocal() as db:
            db.add(PendingEvent(
    ...
    def test_late_scanner_timestamp_after_return_does_not_release_kiz_twice(self):
        ...
        self.assertEqual(self.client.post(
            f"/api/v1/returns/{first_order_id}",
            json={
                "return_reference": "WH-R-LATE-410",
                "returned_by": "warehouse-pc",
                "confirmed_items": self.confirmed_return_items(first_item_id, blocks=1, pieces=20),
            },
        ).status_code, 200)
        ...
        late_re_outbound = self.client.post(
            "/api/v1/scans",
            json={
                "order_item_id": second_item_id,
                "code": code,
                "workstation_id": "offline-pc",
                "scanned_at": "2000-01-01T00:00:00+00:00",
            },
        )
        self.assertEqual(late_re_outbound.status_code, 201)

        _third_order_id, third_item_id = self.seed_order(quantity_blocks=1)
        availability = self.client.get(
            "/api/v1/kiz/availability",
            params={"code": code, "order_item_id": third_item_id},
        )
        third_scan = self.client.post(
            "/api/v1/scans",
            json={"order_item_id": third_item_id, "code": code, "workstation_id": "pc-3"},
        )

        self.assertEqual(availability.status_code, 200)
        self.assertFalse(availability.json()["available"])
        self.assertEqual(availability.json()["latest_movement_type"], "re_outbound")
        self.assertEqual(third_scan.status_code, 409)
        with self.SessionLocal() as db:
            movements = db.execute(
                select(KizMovement)
                .join(KizCode, KizMovement.kiz_id == KizCode.id)
                .where(KizCode.code == code)
                .order_by(KizMovement.occurred_at, KizMovement.id)
            ).scalars().all()
    ...
    def legacy_mark_return_exports_archive_and_returns_to_google_sheets_best_effort(self):
        order_id, item_id = self.seed_order(status="completed", scanned_blocks=2, item_status="completed")
        with self.SessionLocal() as db:
            order = db.get(Order, uuid.UUID(order_id))
            order.raw_payload = {
    ...
    def test_import_creates_grouped_order_items_and_history(self):
        ...
        self.assertNotIn("google_sheets_status", payload)
        ...
        self.assertNotIn("google_sheets", history.json()[0]["raw_payload"])
        with self.SessionLocal() as db:
            event_types = {row.event_type for row in db.execute(select(PendingEvent)).scalars().all()}
            self.assertFalse(any(event_type.startswith("google_") for event_type in event_types))
    ...
    def legacy_import_keeps_backend_data_when_google_sheets_export_fails(self):
        rows = [
            {
                "Дата отгрузки": "30.05.2026",
                "Тип оплаты": "cash",
    ...
    def legacy_import_queue_failure_rolls_back_backend_data_and_intent_together(self):
        rows = [
            {
                "Дата отгрузки": "30.05.2026",
                "Тип оплаты": "cash",
    ...
    def legacy_duplicate_backend_import_still_can_backfill_google_sheets(self):
        row = {
            "Дата отгрузки": "30.05.2026",
            "Тип оплаты": "cash",
    ...
    def test_changed_address_with_same_source_import_id_does_not_create_backend_duplicate(self):
        ...
        self.assertNotIn("google_sheets_status", second.json())
        with self.SessionLocal() as db:
            orders = db.execute(select(Order)).scalars().all()
    ...
    def legacy_google_sheets_export_updates_gps_address_for_existing_import_id(self):
        class FakeSheet:
            def __init__(self):
                self.updates = []
    ...
    def legacy_google_sheets_export_updates_not_found_address_for_existing_import_id(self):
        class FakeSheet:
            def __init__(self):
                self.updates = []
    ...
    def legacy_google_sheets_export_updates_coordinate_address_by_business_key_when_ids_changed(self):
        class FakeSheet:
            def __init__(self):
                self.updates = []
    ...
    def legacy_google_sheets_export_does_not_update_ambiguous_business_key_address(self):
        class FakeSheet:
            def __init__(self):
                self.updates = []
    ...
    def test_logistics_report_uses_shipment_date_coordinates_and_prices(self):
        ...
        self.assertEqual(sheet["D2"].value, "logistics-source-order")
        self.assertEqual(sheet["F2"].value, "Logistics Client")
        self.assertEqual(sheet["I2"].value, "Rep One")
        self.assertEqual(sheet["S2"].value, "41.31")
        self.assertEqual(sheet["T2"].value, "69.27")
        self.assertEqual(sheet["U2"].value, "Tashkent Address")
        self.assertEqual(sheet["V2"].value, datetime(2026, 5, 30, 10, 0))
        self.assertEqual(sheet["W2"].value, datetime(2026, 5, 30, 18, 0))
        self.assertEqual(sheet["AC2"].value, "Chapman Brown OP 20")
        self.assertEqual(sheet["AE2"].value, 0)
        self.assertEqual(sheet["AF2"].value, 0)
        self.assertEqual(sheet["AG2"].value, 20)
        workbook.close()

    def test_logistics_report_splits_skladbot_and_smartup_ids(self):
        rows = [
            {
                "Дата отгрузки": "2026-07-08",
                "Тип оплаты": "Терминал",
                "Клиент": "Smartup Logistics Client",
                "Адрес": "Smartup Address",
                "Координаты": "41.30, 69.20",
                "Торговый представитель": "ТП-1 Умид",
                "Товары": "Chapman Brown OP 20",
                "Кол-во блок": "3",
                "ID заказа": "smartup:259704266",
                "ID импорта": "smartup:259704266:line-1:1",
                "Номер заявки SkladBot": "WH-R-204528",
            },
        ]
        imported = self.client.post("/api/v1/imports", json={"source": "smartup_auto", "filename": "smartup.xlsx", "rows": rows})
        self.assertEqual(imported.status_code, 201)

        report = self.client.get("/api/v1/logistics/report?shipment_date=2026-07-08")

        self.assertEqual(report.status_code, 200)
        workbook = openpyxl.load_workbook(BytesIO(report.content), data_only=True)
        sheet = workbook["Orders"]
        self.assertEqual(sheet["B2"].value, "WH-R-204528")
        self.assertEqual(sheet["C2"].value, "smartup:259704266")
        self.assertEqual(sheet["D2"].value, "smartup:259704266")
        self.assertEqual(sheet["I2"].value, "ТП1 Умид")
        workbook.close()
    ...
    def test_logistics_report_uses_quantity_blocks_for_boxes(self):
        ...
        sheet = workbook["Orders"]
        self.assertEqual(sheet["AC2"].value, "Chapman Brown OP 20")
        self.assertEqual(sheet["AG2"].value, 50)
        workbook.close()
    ...
    def test_logistics_report_uses_saved_client_point_timeslot(self):
        ...
        workbook = openpyxl.load_workbook(BytesIO(report.content), data_only=True)
        sheet = workbook["Orders"]
        self.assertEqual(sheet["F2"].value, "Timeslot Legal Entity")
        self.assertEqual(sheet["V2"].value, datetime(2026, 5, 30, 8, 30))
        self.assertEqual(sheet["W2"].value, datetime(2026, 5, 30, 11, 45))
        workbook.close()
    ...
    def test_import_updates_client_point_address_and_keeps_timeslot_by_client(self):
        ...
        sheet = workbook["Orders"]
        self.assertEqual(sheet["F2"].value, "Timeslot Legal Entity")
        self.assertEqual(sheet["U2"].value, "New Timeslot Address")
        self.assertEqual(sheet["V2"].value, datetime(2026, 5, 31, 8, 30))
        self.assertEqual(sheet["W2"].value, datetime(2026, 5, 31, 11, 45))
        workbook.close()
    ...
    def test_logistics_report_keeps_unrouteable_orders_on_separate_sheet(self):
        ...
        self.assertEqual(sheet["F2"].value, "Route Client")
        self.assertEqual(sheet["U2"].value, "Tashkent Address")
        self.assertEqual(sheet["AC2"].value, "Chapman Brown OP 20")
        ...
        self.assertEqual(problem_rows["No Coordinates Client"][4], "no-coordinates-order")
        self.assertEqual(problem_rows["No Coordinates Client"][5], "Нет координат")
        self.assertEqual(problem_rows["Invalid Coordinates Client"][1], "Invalid Coordinates Address")
        self.assertEqual(problem_rows["Invalid Coordinates Client"][5], "Невалидные координаты")
        workbook.close()
    ...
    def test_logistics_report_normalizes_three_part_coordinates(self):
        ...
        self.assertEqual(sheet["S2"].value, "41.214609")
        self.assertEqual(sheet["T2"].value, "69.223027")
        workbook.close()
    ...
    def legacy_reconciliation_report_endpoint_records_google_down_as_mirror_issue(self):
        with self.SessionLocal() as db:
            order = Order(
                payment_type="terminal",
    ...
    def test_health_is_lightweight_and_readiness_reports_sanitized_db_queue_status(self):
        ...
        self.assertNotIn("google_mirror", payload)
    ...
    def legacy_readiness_keeps_hot_path_ok_when_only_google_mirror_is_degraded(self):
