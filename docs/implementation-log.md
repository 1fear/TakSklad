...
- Daily XLSX формирует ровно четыре клиентских листа: `Сводка`, `Заявки`, `Товары заявок`, `Коды маркировок`. Единственный лист кодов содержит только активные коды, однозначно связанные с заявками этого SkladBot daily; отдельная scheduled KIZ-доставка и её caption удалены, transfer-KIZ `on_completion` не менялся.
...
- Причина: технические листы, повторно добавленные 17.07.2026, нарушили прямой клиентский контракт Антона и перегрузили daily XLSX.
- `build_skladbot_daily_report_xlsx()` формирует ровно три листа: `Сводка`, `Заявки`, `Товары заявок`.
- Internal `movements`, `stock`, `coverage`, `excluded_requests`, `date_diagnostics`, `errors` продолжают собираться и участвовать в проверках полноты, но не выводятся отдельными листами.
- `Сводка` снова не содержит отдельного блока движений.
- Telegram send, deploy и production-write в рамках правки не выполнялись.
...
- Инварианты:
  ...
  - заказы, статусы, остатки, КИЗы, Google Sheets строки и SkladBot API не меняются.
- Проверено:
