...
(`backend/app/orders_service.py:435-436`), это и есть основание для безопасного replay
...
> **Выполнена 2026-08-06, коммит `2aa7c3f`.** Тексты согласованы с Антоном
>
> Сверх исходного описания сделано два пункта, оба закрывают найденные при
> реализации дыры:
>
> 1. **Отмена последнего КИЗа знает про очередь** Кнопка брала последний
>    подтверждённый сервером код, поэтому после офлайн-скана отменила бы чужой
>    код Теперь сначала снимается свежий элемент очереди, как в десктопном
>    `undo_backend_scan`
> 2. **Расхождение ответа сервера в очередь не уходит** `ScanNotQueueableError`:
>    backend ответил, но другим кодом, это fail-closed, а не обрыв связи
>    Поймано существующим тестом `fails closed when the authoritative response
>    returns another code`
>
> Прогресс позиции и признак завершённости считаются через `projectItemProgress`,
> иначе офлайн-скан выглядит как несостоявшийся и оператор сканирует блок дважды
>
> Пять мутаций проверены на закоммиченном коде, каждая роняет ровно один тест:
> снятый гард дубля, снятый fail-closed, завершение без учёта очереди, отмена без
> учёта очереди, прогресс без учёта очереди
>
> Не входит в Task 6 и остаётся в Task 7: список заблокированных сканов с
> причиной словами
