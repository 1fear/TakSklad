...
| Файл | Статус |
|---|---|
| `order-position-merge-2026-07-31.md` | `ACTIVE`: склейка позиций заказа по «ID заказа» и товару, починка накопленных дублей, ограничения ключа `item_key`. |
| `smartup-deal-split-design-2026-07-30.md` | `ACTIVE`: разделение заказов Smartup по `deal_id` и пропуск повторных сделок. |
| `kiz-lifecycle-integrity-architecture-plan-2026-07-30.md` | `ACTIVE`: план целостности жизненного цикла КИЗов. |
