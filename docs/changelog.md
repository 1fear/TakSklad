...
- Клиентская доставка в `22:00 Asia/Tashkent` содержит ровно одно текстовое сообщение и один XLSX с листами `Сводка`, `Заявки`, `Товары заявок`, `КИЗы заявок`, `КИЗы за день`; отдельный daily-файл КИЗ удалён.
- В `Заявки` добавлена колонка `КИЗов`. Для однозначно связанной заявки показывается число активных кодов, включая `0`; при неоднозначной связи ячейка остаётся пустой.
- `КИЗы заявок` включает все текущие активные сканы однозначно связанной выполненной заявки независимо от даты и типа оплаты. `КИЗы за день` включает только активные терминальные сканы выбранного бизнес-дня.
