...
### Короб Chapman KSSL засчитывается как 50 блоков

- `AGGREGATE_BOX_PRODUCT_PREFIXES` в `backend/app/scan_quantities.py`, `src/taksklad/scan_quantities.py` и `frontend/src/features/warehouse/scanQuantities.ts` получил коробочные GTIN KSSL: `4006396104205` это `brown:kssl`, `4006396104236` это `green:kssl`. До этого короб KSSL отбивался и на десктопе, и на backend как чужой товар (`scan_product_mismatch`), склад сканировал KSSL поблочно
- GTIN короба это следующий номер после штучного с пересчитанной контрольной цифрой, как у шести старых SKU. Оба кода подтверждены реальными коробами 24.09.2026 (`0104006396104205…` и `0104006396104236…`, по 67 знаков), в тестах лежат настоящие коды
- Короб проходит только длиной 67 знаков (`length_for_gtin`), только на позицию своего цвета и формата (`aggregate_box_product_mismatch`) и только если влезает в остаток позиции (`aggregate_box_exceeds_plan`)
- Десктоп сверяет код сам до отправки на backend, поэтому на складе короб KSSL заработает только с новой сборкой десктопа
- Покрытие: `tests/test_scan_quantities.py` (короб KSSL это 50 блоков своего цвета на десктопе и backend, чужая позиция отбивается, обрезок до длины блока отбивается), `tests/test_kssl_end_to_end.py` (короб закрывает позицию из 50 блоков одним сканом, не влезает в позицию из трёх и двух блоков, не садится на другой цвет), `frontend/src/__tests__/scanQuantities.test.ts`

## 2026-09-11

### Десктоп 2.0.55: склад видит, что возврат по перечислению ушёл на одобрение

- `APP_VERSION` в `src/taksklad/config.py` поднят с `2.0.54` до `2.0.55`. Сам текст отказа появился в `src/taksklad/app_returns.py` ещё в PR #173 и #175, но номер версии тогда не подняли, поэтому собранный десктоп остался на ревизии `63f4506` от 20.07.2026, а канал обновления считал парк актуальным и новую сборку не предлагал
- Что это значило на складе: backend с 08.09 отвечает на возврат заказа с оплатой «Перечисление» кодом `409 return_approval_requested`, а старая `format_returns_error` ветки на 409 не имеет вовсе и показывает общую строку «Не удалось выполнить запрос возвратов». 10.09.2026 по накладной WH-R-232011 кладовщик нажал возврат двенадцать раз подряд, каждый раз получая этот текст, при том что запрос владельцу ушёл с первого нажатия и заявка возврата не должна была создаваться до одобрения
- Backend, telegram-воркер и веб-панель нужного поведения уже несут, проверено в боевых контейнерах: кнопки «Одобрить возврат» в `vds-telegram-worker-1`, гейт в `vds-backend-api-1`, русский текст в собранном ассете `vds-frontend-1`. Отставал только Windows-клиент
- Статус контракта в `docs/server-desktop-release-separation.md` переведён на `2.0.55`
- Покрытие: `tests/test_desktop_ui_contract.py`, `test_returns_error_names_owner_approval_for_transfer_payment` сверяет оба кода отказа с текстами
- Публичный `version.json` этим PR не меняется: канал промоутится отдельным PR после сборки, подписи и сверки SHA-256 артефактов, как это было сделано парой #77 и #78
...
### Склад собирает заказы накануне рабочего дня логистики, а не только в свои рабочие дни

- `smartup_export_day_decision` в `backend/app/smartup_auto_import.py` решает, собирает ли склад заказы в этот день: рабочий день по календарю логистики остаётся рабочим, суббота и воскресенье остаются выходными склада, а будний день, закрытый вручную в календаре логистики, становится днём выгрузки, если это последний будний день перед ближайшим рабочим днём логистики
- `run_due_smartup_auto_imports` и `run_due_smartup_logistics_reports` берут это решение вместо прямой проверки `is_logistics_non_working_day`: раньше ручной нерабочий день логистики гасил и слоты выгрузки, и финальный отчёт логистики
- Простой на закрытом дне виден в результате воркера: `reason=logistics_non_working_day` плюс `next_logistics_day` и `export_day`, выходные по-прежнему дают `weekday_disabled`; в день выгрузки накануне воркер пишет в лог строку `Smartup export day`
- Причина: 20.08.2026 в календаре логистики отмечены нерабочими 31.08, 01.09 и 02.09, воркер три дня подряд возвращал `idle / weekday_disabled`, и слоты 02.09 не выгрузили заказы на 03.09, хотя 03.09 логистика работает; правило от 02.09.2026: если завтра логистика работает, склад собирает заказы сегодня
- Покрытие: `tests/test_smartup_auto_import.py`, шесть новых проверок: слот на закрытой среде перед рабочим четвергом, простой в понедельник при закрытых вторнике и среде, воскресенье перед рабочим понедельником остаётся выходным, пятничный праздник перед обычным понедельником покрывает субботу и воскресенье, финальный отчёт на закрытом дне накануне рабочего, таблица решений с 28.08 по 03.09
...
- `normalize_import_row` (`backend/app/imports_service.py`) кладёт сделку из колонки «Smartup ИД заказа» в ключ заказа, поэтому два разных заказа Smartup на одного клиента, дату, адрес и представителя остаются двумя заказами склада и получают две заявки SkladBot, а склад пикает их по отдельности. Прежний хеш той же строки считается рядом как `legacy_order_key`
- Заказ для строки ищет `resolve_import_order_for_row`: сначала ключ со сделкой, затем прежний ключ, и заказ прежнего ключа берёт строку только когда его сделка совпадает или ещё не заполнена (`order_accepts_import_row_identity`). Дозаказ той же сделки продолжает вливаться в заказы, заведённые до этой правки, а чужая сделка заводит свой заказ
- Предпросмотр импорта и `find_active_order_for_import_rows` ищут заказ тем же способом, поэтому бот показывает тот заказ, в который файл потом и попадёт. Блокировки строк и префетч берут оба ключа
- Колонка `Smartup deal_id` автоимпорта работает как раньше: при ней ключ не меняется, поэтому хеши заказов `smartup_auto` остались прежними
- Причина: 26.08.2026 два файла на 27.08 по одному клиенту (сделки Smartup 272382350 и 283500698) слились в один заказ на 12 блоков, а заявка на 4 блока осталась одна, потому что синтетический «ID заказа» в шаблоне у разных сделок совпадает
- Решение от 12.08.2026 «группировка заказов не трогается» (`docs/superpowers/specs/2026-08-12-client-search-and-smartup-id-design.md`) этой правкой отменено осознанно
- Покрытие: `tests/test_import_smartup_order_identity.py` (7 проверок: два заказа с двумя заявками, дозаказ той же сделки, заказ прежнего ключа со своей и с чужой сделкой, повторный файл, стабильность хеша без сделки), в `tests/test_import_client_point_prefetch.py` переписаны `test_second_deal_becomes_its_own_order_with_its_own_identity` и `test_repeat_import_fills_empty_identity_and_never_overwrites_it`

## 2026-08-18

### Лист `Товары заявок` дейли получил колонку `Тип оплаты`

- Колонка вставлена пятой, сразу после `Дата выгрузки`, лист вырос с 11 до 12 колонок: `Заявка`, `Smartup ID`, `Тип`, `Дата выгрузки`, `Тип оплаты`, `Юрлицо/точка`, `Торговый представитель`, `Товар`, `Штрихкод`, `Код маркировки`, `Тип кода`, `Блоков по коду`.
- Значение берётся тем же `payment_type_from_comment`, что и на листе `Заявки`: первое слово первой строки комментария заявки, если оно называет тип оплаты. Один и тот же заказ показывает одинаковое значение на обоих листах, комментарий без типа оплаты даёт пустую ячейку.
- Ширины шаблона на листе сдвинуты на одну колонку, новая колонка получила ширину 14, как `Тип оплаты` на листе `Заявки`.
- Покрытие: `tests/test_skladbot_daily_report.py`, проверки заголовков шаблона, значений `Терминал` и `Перечисление` на листе и текстовых литералов в сдвинутых колонках.

## 2026-08-13

### Дейли SkladBot переведён на согласованный шаблон рабочей группы

- Клиентский XLSX содержит три листа: `Сводка`, `Заявки`, `Товары заявок`. Лист `Коды маркировок` удалён, вместе с ним из отчёта ушло время скана, других колонок со временем в книге нет.
- Лист `Заявки` сокращён с 23 до 15 колонок: убраны `ID`, `Категория`, `В архиве`, `Дата обновления`, `Раб зона`, `Клиент SkladBot`, `Коды маркировки`, `Отклонение`, `Товаров`, `Причина включения`. Причина включения и coverage остаются во внутренней диагностике.
- Колонка `Комментарий` разобрана на `Тип оплаты` (первое слово комментария: `Перечисление` или `Терминал`), `Рабочий номер` и `Личный номер`. Строки без телефонов дают пустые ячейки, отчёт не подставляет данные из справочника ТП.
- Лист `Товары заявок` сокращён с 15 до 11 колонок и выводит одну строку на код маркировки: `Код маркировки`, `Тип кода` (`Блок` или `Короб`) и `Блоков по коду` вместо `Блоков план`, `Принято факт`, `Блоков факт`, `Отклонение`. Убраны `ID заявки`, `Раб зона`, `Артикул`.
- Код привязывается к строке товара по product key самого кода (`scan_code_product_key` против `product_key_from_name`), поэтому коробочный GTIN тоже находит свой товар. Код без совпавшего товара выводится отдельной строкой заявки, потерять код нельзя.
- Проверено на дейли за 12.08.2026: 597 кодов на входе, 597 на выходе, пары заявка + код совпадают, 146 заявок и 355 пар заявка + товар сохранены.
- Покрытие: `tests/test_skladbot_daily_report.py` (`test_client_workbook_headers_match_agreed_daily_template`, `test_request_products_sheet_expands_codes_and_never_drops_unmatched`, `test_requests_sheet_splits_comment_into_payment_and_phones`).

## 2026-07-22

### Combined SkladBot daily with terminal KIZ and guarded catch-up

- Клиентская доставка в `22:00 Asia/Tashkent` содержит ровно одно текстовое сообщение и один XLSX с листами `Сводка`, `Заявки`, `Товары заявок`, `Коды маркировок`; отдельный daily-файл КИЗ удалён.
- В `Заявки` добавлены колонки `КИЗов` и `Коды маркировки`. Для однозначно связанной заявки показываются число и сами активные коды, включая `0` при доказанном отсутствии; при неоднозначной связи ячейки остаются пустыми.
- `Коды маркировок` включает все текущие активные коды однозначно связанной выполненной заявки, вошедшей в SkladBot daily, независимо от даты и типа оплаты. Несвязанные терминальные сканы в клиентский файл не выводятся.
- Связь строится только по однозначной canonical-паре `SkladBot ID + WH-R/WR номер`; эвристики по клиенту, дате, товару и Smartup ID не используются. Lifecycle `return/undo/reset`, конфликт владельца и duplicate active code обрабатываются fail-closed до Telegram.
- Обычная открытая заявка, найденная только по `created_at` за день отчёта, остаётся диагностической строкой, но больше не делает coverage `partial`; реальные API/detail/pagination/date ошибки продолжают блокировать отправку.
- Добавлен durable одноразовый `manual_catchup` с no-send dry-run, запретом автоматического повтора после начала Telegram и строго ограниченным production recovery path для двух согласованных дат.

## 2026-07-20

### Daily SkladBot workbook restored to three client sheets

- Клиентский XLSX снова содержит только `Сводка`, `Заявки`, `Товары заявок`.
- Листы `Движения`, `Остатки`, `Покрытие`, `Исключенные заявки`, `Диагностика дат`, `Ошибки` удалены из workbook; внутренний сбор данных, coverage и fail-closed scheduled blocker сохранены.
- Из `Сводка` удален технический блок движений; остались категории заявок и актуальный остаток.
- Точный состав листов и отсутствие технических строк закреплены focused XLSX-тестами.
...
**Файлы:** `backend/app/skladbot_daily_report.py`, `backend/app/telegram_worker.py`, `tests/test_skladbot_daily_report.py`, `deploy/vds/.env.example`, `README.md`, `backend/README.md`, `docs/report-source-rules.md`, `.supergoal/daily-skladbot-audit-closeout-2026-07-06/*`, `docs/implementation-log.md`, `docs/changelog.md`.

**Что стало:**

- Daily report wrapped SkladBot client in a read-only boundary: write helpers and write POST `/requests` are blocked from report collection.
- Read-style POST is explicit for `/warehouse/transactions`, `/products`, `/report/stock`; retry policy applies only there and records coverage counters.
- Movements/products/stock limit-hit cases mark coverage `partial` and block scheduled Telegram send.
- Included rows with conflicting `unloading_date` and movement date remain visible, but make coverage `partial`.
- Manual `/skladbot_daily` blocks partial reports by default; explicit `--allow-partial` marks the file/message as incomplete and does not act as scheduled recovery.
- Same-day corrected scheduled reports do not auto-send; existing event is marked `manual_recovery_required`.
- `Сводка` now labels start stock as `Расчетный начальный остаток` and documents that it is a formula, not a historical stock snapshot.
- Deploy, Telegram send, SkladBot write calls, production DB writes, commit and push were not performed.

**Проверки:**

- `PYTHONPATH=. ./.venv/bin/python -m unittest tests.test_skladbot_daily_report` - 69 tests OK.
- `PYTHONPATH=. ./.venv/bin/python -m unittest tests.test_backend_telegram_import` - 71 tests OK.
- `PYTHONPATH=. ./.venv/bin/python -m unittest tests.test_skladbot_daily_report tests.test_backend_telegram_import` - 140 tests OK.
- Final compile/diff checks are recorded in `.supergoal/daily-skladbot-audit-closeout-2026-07-06/TEST_RESULTS.md`.

## 2026-07-05

### Daily SkladBot partial-report send block

**Файлы:** `backend/app/skladbot_daily_report.py`, `backend/app/telegram_worker.py`, `tests/test_skladbot_daily_report.py`, `docs/report-source-rules.md`, `docs/implementation-log.md`, `docs/changelog.md`, `.supergoal/daily-skladbot-bvytBP/PARTIAL_REPORT_FIX_VERIFICATION.md`, `.supergoal/daily-skladbot-bvytBP/PARTIAL_REPORT_FIX_RESULTS.md`, `.supergoal/daily-skladbot-bvytBP/PARTIAL_REPORT_FIX_DEPLOY_PLAN.md`, `.supergoal/daily-skladbot-bvytBP/PARTIAL_REPORT_2026-07-05_FINAL_FACTS.md`.

**Что стало:**

- Scheduled SkladBot daily block теперь останавливает Telegram send и daily reconciliation не только при `coverage_status != complete`, но и при `0` operational rows + diagnostic/excluded rows.
- Detail fetch порядок разделен по list-level bucket: in-scope, unknown date, diagnostic/out-of-scope sample. Known out-of-scope rows больше не могут съесть основной detail budget.
- Coverage получил счетчики `detail_attempted_in_scope`, `detail_attempted_unknown_date`, `detail_attempted_out_of_scope_sample`, `out_of_scope_skipped_without_detail`, `in_scope_candidates_not_detailed`, `detail_limit_reached`.
- Page counters разделяют aggregate list pages и per-type guard через `list_pages_fetched`, `max_pages_per_request_type`, `list_page_guard_max_total`, `detail_pages_fetched`, `total_http_pages_fetched`.
- Deploy не выполнялся, Telegram не отправлялся, production DB не менялась.

**Проверки:**

- `PYTHONPATH=. ./.venv/bin/python -m unittest tests.test_skladbot_daily_report` - 46 tests OK.
- Exact contract tests для partial block, complete send, detail budget, max-pages split, timeout, sendDocument failure, same-day no retry и safe progress payload - OK.

### Daily SkladBot strict date coverage

**Файлы:** `backend/app/skladbot_daily_report.py`, `backend/app/telegram_worker.py`, `tests/test_skladbot_daily_report.py`, `.supergoal/daily-skladbot-bvytBP/PHASE_0_CURRENT_CONTRACT.md`, `docs/report-source-rules.md`, `docs/implementation-log.md`, `docs/changelog.md`.

**Что стало:**

- `/requests` для daily SkladBot читается page-based через `page + limit`, без `offset`, с dedupe и stop guards.
- Primary daily scope теперь `Дата выгрузки` / `unloading_date` или movement date за дату отчета; `created_at` остался diagnostic signal.
- Operational totals включают только completed+archived заявки, прошедшие date eligibility.
- Excluded/out-of-scope/status/API-error строки не исчезают: они попадают в coverage data и XLSX листы `Контроль покрытия`, `Исключенные заявки`, `Диагностика дат`, `Ошибки`.
- Telegram summary показывает coverage status, included/excluded counts, API errors и warning для partial/failed отчета.
- Scheduled reported-request registry key теперь включает report date, chat, mode, report kind и report version hash.
- Deploy не выполнялся.

**Проверки:**

- `PYTHONPATH=. ./.venv/bin/python -m unittest tests.test_skladbot_daily_report` - 31 tests OK.
- `PYTHONPATH=. ./.venv/bin/python -m unittest tests.test_skladbot_daily_report tests.test_backend_telegram_import` - 102 tests OK.
...
- `pending_google_exports` в totals web-таблицы считается по тем же видимым строкам, что и остальные totals.
- `google_sheets_skladbot_export` не попадает в admin-table pending totals, как и в row-level pending.
- Smartup preview получает тот же `source_batch_key`, что и реальный import.
- Late split order key для Smartup больше не зависит от SHA XLSX: повторная выгрузка той же части с изменившимся файлом дописывается в тот же split-order.
- Web-панель после server-side фильтров больше не фильтрует таблицу повторно на клиенте, чтобы `load more`, selection и bulk actions не расходились с backend offset.

**Проверки:**

- `PYTHONPATH=. ./.venv/bin/python -m unittest tests.test_backend_api_persistence.BackendApiPersistenceTests.test_admin_table_filters_status_before_limit tests.test_backend_api_persistence.BackendApiPersistenceTests.test_admin_table_counts_pending_google_exports_from_order_ids tests.test_backend_api_persistence.BackendApiPersistenceTests.test_smartup_late_export_splits_when_existing_order_already_has_skladbot_request tests.test_smartup_auto_import.SmartupAutoImportTests.test_preview_delivery_groups_uses_same_source_batch_key_as_import` - OK.
- `PYTHONPATH=. ./.venv/bin/python -m unittest tests.test_backend_api_persistence tests.test_smartup_auto_import` - 153 tests OK.
- `PYTHONPATH=. ./.venv/bin/python -m unittest tests.test_release_go_no_go tests.test_vds_acceptance_scripts tests.test_ci_cd_workflows tests.test_backend_skladbot_request_dry_run` - 47 tests OK.
- `npm --prefix frontend run build` - OK.
- `PYTHONPATH=. ./.venv/bin/python tools/release_preflight.py --skip-network` - OK.
- `PYTHONPATH=. ./.venv/bin/python -m alembic -c backend/alembic.ini heads` - `20260701_0006 (head)`.
- `TAKSKLAD_ENV_FILE=.env.example docker compose --env-file deploy/vds/.env.example -f deploy/vds/docker-compose.yml config --quiet` - OK.
- `git diff --check` - OK.

### Smartup late exports split after SkladBot link

**Файлы:** `backend/app/imports_service.py`, `backend/app/smartup_auto_import.py`, `backend/app/skladbot_request_dry_run.py`, `tests/test_backend_api_persistence.py`, `tests/test_smartup_auto_import.py`, `tests/test_backend_skladbot_request_dry_run.py`, `docs/implementation-log.md`, `docs/changelog.md`.

**Что стало:**

- Если Smartup приносит новую выгрузку по тому же юрлицу/адресу/дате, но старый backend order уже связан с SkladBot, новая строка не добавляется в старый order.
- Если SkladBot-заявка для старого backend order еще только стоит в очереди `skladbot_request_create`, новая Smartup-выгрузка тоже отделяется в новый order.
- Для поздней выгрузки создается отдельный backend order со стабильным split-key по `order_key + source_batch_key`.
- Новый backend order идет в обычный SkladBot create flow и получает отдельную WH-заявку.
- Smartup create больше не задерживается до финального слота.
- Preview import использует тот же split-key, что и реальный import.
...
- Runtime smoke confirmed split behavior and absence of old `not_before` defer code.
- Pending-create follow-up restore point: `/opt/stacks/taksklad/restore_points/pre-smartup-pending-split-20260701T125238Z`.
- Pending-create follow-up Postgres backup: `/opt/taksklad/backups/postgres/taksklad-postgres-20260701T125238Z.sql.gz`.
- Pending-create follow-up rebuilt/recreated: `backend-api`, `smartup-auto-import-worker`.
- Runtime smoke confirmed `skladbot_request_create` pending event now forces a split; `excel` imports do not split.
...
**Файлы:** `backend/app/skladbot_request_dry_run.py`, `tests/test_backend_skladbot_request_dry_run.py`, `docs/implementation-log.md`, `docs/changelog.md`.

**Что стало:**

- Перед формированием SkladBot dry-run/create payload одинаковые ready SKU внутри одной заявки схлопываются по `product_data_id + barcode + is_main_barcode`.
- Количество блоков суммируется, например две строки `Chapman Green OP 20` по 1 блоку становятся одной SkladBot-позицией `amount=2`.
- Исходные строки импорта не удаляются: evidence остается в `OrderItem`/Google, а dry-run product хранит `source_products`.
...
- Runtime aggregation smoke: `[(2430805, 2, 2), (2189392, 1, 1)]`.
- Public `/health` OK; `/ready` remains degraded by old unrelated queue/mirror items.
- Fresh logs for rebuilt services: no errors.

### Controlled GitHub CI/CD

**Файлы:** `.github/workflows/ci.yml`, `.github/workflows/deploy-production.yml`, `deploy/vds/deploy_from_git.sh`, `tests/test_ci_cd_workflows.py`, `docs/deploy-rollback-runbook.md`, `docs/implementation-log.md`, `docs/changelog.md`.

**Что стало:**

- Добавлен CI на `push main`, `pull_request main` и ручной запуск.
- Добавлен production CD только через ручной GitHub Actions workflow `Deploy Production`.
- Production deploy создает restore point, делает Postgres backup, применяет Alembic migration, пересобирает выбранные compose-сервисы и проверяет `/health`, `/ready`, optional/required acceptance и свежие логи.
- Обычный `git push` по-прежнему не выкатывает production.
- Секреты VDS остаются в GitHub Secrets/Environment и не попадают в репозиторий.
- Restore point в deploy-скрипте исключает `.env*`, `node_modules`, `dist`, `__pycache__` и `*.pyc`.
- Backup/restore scripts теперь уважают `TAKSKLAD_ENV_FILE` и `TAKSKLAD_COMPOSE_FILE`, а systemd backup unit использует актуальный `/opt/stacks/taksklad/app`.

**Проверки:**

- `bash -n deploy/vds/*.sh` - OK.
- Ruby YAML parse для GitHub workflows - OK.
...
- `PYTHONPATH=. ./.venv/bin/python -m alembic -c backend/alembic.ini heads` - `20260701_0006 (head)`.
- `git diff --check` - OK.
- Локальный full `unittest discover -s tests` упал только на импорте Tk-dependent desktop tests из-за отсутствующего `_tkinter` в текущем Homebrew Python; GitHub CI устанавливает `python3-tk` перед full discovery.

### YASMINA WH-R-202405 production DB blocks repair

**Файлы:** `docs/implementation-log.md`, `docs/changelog.md`. Код не менялся.

**Что стало:**

- По `"YASMINA GROUP 555" MCHJ` production DB приведена к факту SkladBot: `WH-R-202405` содержит `4` блока.
- Удалена одна лишняя неотсканированная repair-позиция `smartup:257984858:1541071310:1`, которая давала backend `5` блоков против `4` в SkladBot.
- Заказ `a949d657-e20a-4def-944c-f20c560edba9` переведен в `completed`: осталось 4 позиции, все `1/1`.
- SkladBot не изменялся, новая заявка не создавалась.

**Production repair:**

- DB backup: `/opt/taksklad/backups/postgres/taksklad-postgres-20260701T075633Z.sql.gz`, SHA256 `f4512ed14374039cecf3d3f73682e63314f1ae5b28988256d9ecd3c8f433659f`.
- Evidence snapshot: `/opt/stacks/taksklad/repair_evidence/yasmina-wh-r-202405-blocks-before-20260701T075719Z.json`, SHA256 `c3f0e5b736a31754b712308ff4bc5c58f71e0fba72a16c2a65d7b50ca33268f0`.
- Google cleanup/archive events completed: `google_sheets_delete_import_records_export`, `google_sheets_archive_export`.

**Проверки:**

- Production SQL: `db_blocks=4`, `scanned_blocks=4`, `item_count=4`, linked SkladBot amounts `[1, 1, 1, 1]`.
- Rebuilt SkladBot dry-run for repair import: `linked_mismatch=0`, `events_queued=0`.
- New Google/backend conflicts for this order after repair: `0`.
- Public `/health` OK, version `2.0.25`; `/ready` degraded only by old unrelated queue/mirror items.

### SkladBot representative comments and daily report column

**Файлы:** `backend/app/representative_contacts.py`, `backend/app/skladbot_request_dry_run.py`, `backend/app/skladbot_return_requests.py`, `backend/app/skladbot_daily_report.py`, `backend/app/models.py`, `backend/app/health_service.py`, `backend/sql/001_initial_schema.sql`, `backend/migrations/versions/20260701_0006_representative_contacts.py`, `tools/import_representative_contacts.py`, tests and docs.

**Что стало:**

- Добавлен DB-справочник торговых представителей с рабочим/личным телефоном и рабочей зоной.
- Добавлен импортёр XLSX-справочника: `tools/import_representative_contacts.py`.
- При создании SkladBot-заявок TakSklad пишет в comment тип оплаты, ТП и найденные телефоны.
- Ежедневный SkladBot XLSX показывает `Торговый представитель` на листах `Заявки` и `Товары заявок`.
- Реальные телефонные номера не добавляются в репозиторий.
...
**Файлы:** `backend/app/imports_service.py`, `backend/app/kiz_reports_service.py`, `backend/app/smartup_auto_import.py`, `tests/test_smartup_auto_import.py`, `tests/test_backend_api_persistence.py`, `docs/report-source-rules.md`, `docs/implementation-log.md`, `docs/changelog.md`.

**Что стало:**

- Smartup auto import сохраняет общий `source_batch_key` для всех backend-import, созданных из одного XLSX export-файла.
- KIZ source-files сначала группирует Smartup-позиции по `source_batch_key`; legacy Smartup rows с именем `Терминал ДД.ММ.ГГГГ Часть N.xlsx` объединяются по этому имени.
- Ручные повторные Excel с одинаковым filename продолжают разделяться по `backend_import_id`.
...
- Selective rsync: `backend/app/imports_service.py`, `backend/app/kiz_reports_service.py`, `backend/app/smartup_auto_import.py`, related tests and docs only.
- Rebuilt/recreated: `backend-api`, `smartup-auto-import-worker`.
- Runtime `py_compile` inside both containers - OK.
- Public `/health` OK, version `2.0.25`; public `/ready` remains `degraded` from old `telegram_excel_import` failures and one pending `google_sheets_export`, while DB and migrations are OK.
- VDS `SMARTUP_AUTOMATION_RUNTIME_REQUIRED=1 ./deploy/vds/verify_smartup_automation.sh` returned `status=ok`, pending SkladBot creates `0`.
- Production DB read-only check: `Терминал 01.07.2026 Часть 1.xlsx` is one KIZ source-file row with `batch:legacy-smartup-file` key prefix, `34` items, `47` planned blocks.
...
- Актуальная web-панель 2.0.25 перенесена в `main`: навигация снова содержит `Календарь`, `Smartup` и `История действий` вместо старого набора разделов.
- Сохранен быстрый startup gate: после проверки web-session экран не ждет полный `refreshAll`.
- Стартовый запрос `/api/v1/admin/table` теперь запрашивает первые 500 строк через `limit`, а не весь список позиций одним ответом.
- В таблице добавлена догрузка следующей страницы, чтобы не возвращать 5 МБ данных на первый экран и не ловить браузерный timeout 15 сек.
- Sidebar получил фикс высоты/скролла для длинной панели навигации.

**Проверки:**

- `cd frontend && npm run build` - OK.
- `.venv/bin/python -m unittest tests.test_smartup_auto_import tests.test_google_sheets_sync_worker tests.test_vds_acceptance_scripts tests.test_backend_api_persistence tests.test_backend_telegram_import` - 234 tests OK.
- `.venv/bin/python -m unittest discover -s tests -p 'test_*.py'` - 669 tests OK.
- `.venv/bin/python tools/release_preflight.py` - status `ok`, public backend health OK, version `2.0.25`.
- `TAKSKLAD_ENV_FILE=.env.example docker compose --env-file deploy/vds/.env.example -f deploy/vds/docker-compose.yml config --quiet` - OK.
...
- Windows-приложение проверяет сохранённый служебный доступ на сервере и автоматически заменяет его только после подтверждённого `401/403`.
- Запрос, отклонённый из-за устаревшего служебного доступа, безопасно повторяется один раз после успешной замены ключа.
- Временная недоступность сервера не удаляет локальный ключ и не очищает последний загруженный список заказов.

# 2.0.53 — 2026-07-20

- Завершение заказа сначала фиксируется в локальной долговечной очереди, а затем отправляется в backend.
- При временном `401/503` или обрыве связи событие не теряется и безопасно повторяется без дублирования заказа или КИЗов.
- Успешно подтверждённое завершение удаляется из очереди сразу после ответа backend.

# 2.0.54 — 2026-07-20

- Убран самозапуск нового обновления сразу после завершения предыдущего.
- Плановая синхронизация больше не блокирует и не перерисовывает кнопку «Обновить».
- При выбранном заказе фоновое обновление не запускается.
