...
Этот документ закрывает ручные проверки, которые нельзя честно выполнить из macOS/VDS без
участия реального Telegram-пользователя и Windows-компьютера склада.

Статус 2026-07-02 остаётся историческим production proof прежнего контура.
Для web-кандидата 2026-07-27 локальные, физические и live-гейты вынесены в
[web-warehouse-acceptance.md](web-warehouse-acceptance.md). До отдельной проверки
реального сканера, принтера и оператора соответствующие статусы — `NOT RUN`.
...
Candidate-mode проверяет локальный кандидат `2.0.55`, не утверждая, что public channel уже переключён. Final-mode разрешён только после immutable publication: он скачивает все Windows assets из release manifest, проверяет exact `2.0.55`, `onefile_exe`, SHA256 и attestations.

Он проверяет:

- публичный `https://api.taksklad.uz/health`;
- candidate contract `2.0.55` отдельно от ещё поддерживаемого public channel; final-mode требует опубликованный exact `2.0.55`, `mandatory=true`, `onefile_exe` и immutable ссылки/хеши;
...
1. Запустить подписанный TakSklad v2.0.55 через wrapper выше.
...
- Не перемещать и не перезаписывать существующий tag/release; `v2.0.55` публикуется один раз после final gate.
