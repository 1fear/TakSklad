...
1. Запустить подписанный TakSklad v2.0.46 через wrapper выше.
...
- Не перемещать и не перезаписывать существующий tag/release; `v2.0.46` публикуется один раз после final gate.
