...
```bash
release_dir="/protected/new-download-v2.0.43"
extract_dir="/protected/new-extract-v2.0.43"
mkdir -m 700 "$release_dir"
gh release download v2.0.43 --dir "$release_dir"
...
- Candidate `2.0.43` проверен локально; public channel считается переключённым только после final preflight exact `2.0.43`/`onedir_zip` и immutable release gate.
