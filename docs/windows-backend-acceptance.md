...
До общего cutover проверка выполняется на одном тестовом профиле/ПК. Сам
desktop уже не поддерживает другой источник данных:
...
Ожидаемые поля: `backend_active_orders_source=postgres_backend`,
`queue_stale_processing`, `hot_path_stale_processing`,
`telegram_worker_state`, `telegram_pending_events`.
...
без фактического sync/audit. На backend дополнительно проверить:
