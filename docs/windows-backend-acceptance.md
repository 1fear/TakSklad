...
```bash
release_dir="/protected/new-download-v2.0.44"
extract_dir="/protected/new-extract-v2.0.44"
mkdir -m 700 "$release_dir"
gh release download v2.0.44 --dir "$release_dir"
...
- Candidate `2.0.44` проверен локально; public channel считается переключённым только после final preflight exact `2.0.44`/`onedir_zip` и immutable release gate.
