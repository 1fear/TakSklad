...
```bash
release_dir="/protected/new-download-v2.0.53"
extract_dir="/protected/new-extract-v2.0.53"
mkdir -m 700 "$release_dir"
gh release download v2.0.53 --dir "$release_dir"
...
- Candidate `2.0.53` проверен локально; public channel считается переключённым только после final preflight exact `2.0.53`/`onefile_exe` и immutable release gate.
