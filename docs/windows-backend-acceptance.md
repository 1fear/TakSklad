...
```bash
release_dir="/protected/new-download-v2.0.46"
extract_dir="/protected/new-extract-v2.0.46"
mkdir -m 700 "$release_dir"
gh release download v2.0.46 --dir "$release_dir"
...
- Candidate `2.0.46` проверен локально; public channel считается переключённым только после final preflight exact `2.0.46`/`onefile_exe` и immutable release gate.
