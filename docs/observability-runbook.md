...
Worker freshness uses `last_progress_at` with fallback to `last_cycle_started_at`. SkladBot updates progress only after completed processor phases, event units, HTTP attempts, or bounded retry-backoff chunks. Total retry backoff for one HTTP read is hard-capped at 120 seconds (a lower `SKLADBOT_MAX_TOTAL_RETRY_BACKOFF_SECONDS` is allowed); an oversized provider `Retry-After` fails the cycle instead of keeping readiness green. Progress/result writes are scoped to the cycle correlation ID, so an older overlapping worker cannot keep a newer cycle green. Heartbeat persistence is best-effort and must never change the result of a warehouse operation or hide a hung external call.
