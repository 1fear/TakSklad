...
Текущая версия по коду: `2.0.23`; public `version.json` сейчас в paused rollout `1.1.7`.
...
11. имеет выключенный по умолчанию серверный worker для Smartup `Новые + Терминал`: export, audit, advisory lock по слотам, календарь логистики, status change, backend import, SkladBot queue, Telegram alert при ошибке и финальный логистический отчёт.
