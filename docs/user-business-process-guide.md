...
5. Для маршрутного листа логистики нужны именно координаты, а не только адрес. Если координат нет или они невалидны, заказ не попадает в маршрутный лист `Orders`, но остается в логистическом Excel на листе `Требуют координаты`, чтобы менеджер видел, что нужно исправить.
6. Цена за блок фиксируется как `240 000 сум`. Если импортный файл не содержит готовую сумму по юрлицу или позиции, TakSklad считает сумму сам: `Кол-во блок * 240 000`. Новый маршрутный лист `Orders` цену не выводит; суммы остаются в финансовых/сводных отчетах.
...
8. Кнопка `Выгрузка КИЗов` показывает только те исходные Excel-файлы, по которым все позиции уже полностью отсканированы. Незавершённые файлы не предлагаются к выгрузке. В меню остаются последние 7 импортов по времени загрузки: самый новый показывается первым, а старый сдвигается из списка. Файлы, загруженные через Telegram, стоят выше фоновых Smartup-импортов и не вытесняются ими. Если в разные дни или разными сообщениями пришли файлы с одинаковым названием, TakSklad разделяет их по конкретному импорту и не смешивает КИЗы.
...
Текущий статус: серверный контур Smartup -> TakSklad развернут; в compose/env automation остается выключенной по умолчанию, а production включается только явными флагами после backup, smoke-проверок и runtime verifier.

Рабочая автоматизация:

1. Worker запускается по слотам `12:00`, `15:00`, `17:50`.
2. Штатный Smartup export ориентируется на исходную дату доставки Smartup. В обычный день слот за `01.07.2026` собирает Smartup `delivery_date=02.07.2026`, даже если дата сделки была `30.06.2026`. Если следующая календарная дата нерабочая, слот собирает все Smartup `delivery_date` от следующего дня до ближайшего рабочего дня включительно: пятничный слот перед обычными выходными собирает субботу, воскресенье и понедельник.
...
6. Только после успешного backend import Smartup-заказы переводятся в статус `В ожидании`.
7. SkladBot create-events ставятся через существующий импортный контур только после успешного перевода Smartup-заказов в `В ожидании`.
...
Техническое уточнение: штатные слоты Smartup `order$export` передают `delivery_date` диапазоном от следующего календарного дня до ближайшего рабочего дня логистики включительно и статус `Новые`; фильтр `Терминал` дополнительно enforce-ится локально перед созданием XLSX/import, чтобы неподходящая оплата не попала дальше в pipeline. Ручной `run-once` без `--delivery-date` сохраняет старый режим проверки по дате Smartup-сделки.
...
- завершенный слот повторно не выполняется без ручного административного вмешательства.
