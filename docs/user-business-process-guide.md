...
Печатать сводный лист после каждого готового юр.лица со всеми его позициями.
Маленькое дополнение, при печати сводного листа не должен открыватся браузер, только выходить окно печати, где можно выбрать из доступных принтеров один для печати. Окно печати должно выходить для термопринтера с выбираемым размером (Тут добавь все размеры обычных наклеек для термопринтера, подсказать какие точно не могу, но точно знаю что должен быть 100х100), далее когда выходит окно печати со всеми настройкми печати которые мы обсужали ранее, горячая клавиша кнопки "подтвердить" или "печать" должна быть enter
7. Защититься от дублей и недосканов.
8. Завершить заказ.
9. Напечатать сводный лист. (Перенес этот пункт)
10. Сформировать дневной отчёт. (Тут пробелма в том что склад может сканировать один целый заказ на день, больше одного дня, и тут уже дневной очет будет некорекный, услвно заказ пришел в пятницу на доставку в понедельник, склад может делать его в пятницу, субботу, воскресенье, нужно как то продумать чтобы заказ был корректный)
11. Отправить отчёты, логи и документы в Telegram. (Согласано изменениям в 10 пункте должен выходить корректный отчет всего заказа(Партии заказов)) (Логи копяться в отдельном файле, дата, время, ошибка, выгружается только после того как юзер в телеграмме нажмет кнопку что то вроде "Выгрузить логи", очевидные ошибки по типу повторный киз (Ошибка склада) показываться не должны, потому что мы защитились от дублей и очевидных ошибок, они просто переделывают, и ничего критичного нет. Лог должен быть только критичных ошибок сервера и приложение)

Дополнение, обычно за рабочий день в течение до 18:00 прилетает около 5 заказов excel файлами, после того как все файлы загружены, нужна кнопка сформировать отчет для логистики, шаблон направлю позже. Или можно как еще процесс продумать этот, главное чтобы формировался общий отчет о всех заказах

Также проблема с импортом была в версии 1.1.7, что разные шаблоны кидались, и приложение грузило все позиции, а какие определенные пропускала и не говори об этом, соответвенно целый заказ от юр лица мы пропустили

Важно:

- прямой автоматический импорт из Smartup пока не реализован;
- Smartup сейчас рассматривается как возможный источник Excel-выгрузки;
- TakSklad пока не создаёт заявки в SkladBot;
- SkladBot сейчас используется для чтения и сопоставления номеров заявок;
...
1. Smartup/Excel не обязан содержать отдельный файл или поле даты отгрузки. Это не считается риском: менеджер, который отправляет Excel в Telegram-бот, сам указывает актуальную дату отгрузки перед импортом.
...
5. Для отчёта логистики нужны именно координаты, а не адрес. Если адрес нужен внутри TakSklad для удобства склада, он может храниться отдельно, но логистический Excel должен заполняться координатами как обязательным полем.
...
8. Кнопка `КИЗ по файлам` показывает только те исходные Excel-файлы, по которым все позиции уже полностью отсканированы. Незавершённые файлы не предлагаются к выгрузке.
...
Тут опишу как я вижу и должно быть, пришли заказы, он переслал их в телеграмм бот, он подтянул все данные в дату (лист), далее он начинает заводить заявки в skladbot, сервер сразу начинает тянуть номера заявок рядом с уже загруженными заказами в дате

Склад работает по принципу что в skladbot поступает заявка, они ее обрабатывают и позиции снимаются со стока и основного остатка и печатется накладная с номером заявки и всеми данными, а после они в taksklad пикают кизы и печатают сводный лист (Тут тоже под вопросом, в основном это отмечается просто что заказ уже пропикан кизы, может как то изменить сводный лист потому что вся основная информация есть в накладной от skladbot)

Далее что я хочу чтобы делала система, все заказы которые поступили появляются в дате, там подтягиваются номера заявок skladbot, после когда склад заканчивает день или может другой тригер потому что проблема с днями была уже, все заказы с даты улетали в лист "архив" и хранились там

После дополнение, хочу в приложение изменить дизайн полностью. Для склада, им будет пользоваться пока что только склад, убрать все кнопки лишние, по типу импорта excel, контроль и че то еще кнопка одна рядом была.

Была сухая статистика всех заказов из партии, сколько еще нужно, сколько осталось, список заказов и пиканье кизов. Все

Нововедение что добавляем в приложение после, как то граммотно продумать нужно и сделать максимально просто, интуитивно понятно потому что склад трудно адаптируется, добавить может поле или что то еще связааное с возвратами, как я вижу что это работает. К нам на склад прилетают возвраты, сотрудник склада в taksklad переходит во вкладку возвраты там он пикает просто шк накладной которые доступны в "Архиве", пропикав все он переводит их в возврат. Далее в архиве поля возвратов красятся в красный оттенок не сильно темный, и допольнительное поле к основным что это возврат, и дата возврата на склад, это уже делает приложение, ставит статус возврат, пишут дату пика на возврат, при этом все старые данные архива сохраняются. И еще в лист "Возвраты" попадают целиком вся срока возврата, то есть дата поступления заказа, дата отгрузки до точки, юр лицо и все все все, также дата возврата на склад тоже
...
- передаёт Excel в TakSklad через desktop или Telegram;
...
flowchart TD
    ...
    C -->|Готовый Excel| D
    D --> E{"Способ загрузки"}
    E -->|Desktop| F["Кнопка ИМПОРТ EXCEL"]
...
Текущий статус: прямой API-интеграции Smartup -> TakSklad пока нет.

Рабочий процесс сейчас:

1. Менеджер формирует или получает заказ в Smartup.
2. Менеджер выгружает заказы в Excel.
3. Менеджер проверяет Excel перед загрузкой.
4. Excel загружается в TakSklad.
...
- если адрес получить нельзя, ставится `Адрес не указан` или техническая пометка.
...
### 7.1 Текущая Рабочая Desktop-Линия

Основное рабочее хранилище - Google Sheets, лист `data`.
...
Активный список в desktop строится по невыполненным строкам.

### 7.2 VDS-Линия 2.0

На VDS уже есть staging-основа:
...
В этой линии заказы постепенно переходят в Postgres. Desktop сможет работать с backend через feature flags. До Windows-приёмки это не считается production-режимом.
...
- `Заказы на сегодня` - список активных заказов;
- поле поиска - поиск по клиенту/адресу/оплате/номеру;
- `ОБНОВИТЬ` - перечитать список заказов;
- `ИМПОРТ EXCEL` - загрузить Excel вручную;
- `ТОВАРЫ` - справочник товаров;
- `КОНТРОЛЬ` - контрольная панель;
- `ВЫБРАТЬ ЗАКАЗ` - открыть выбранный заказ для сканирования.

Как пользоваться:

1. Нажать `ОБНОВИТЬ`.
2. Найти нужный заказ в списке.
...
- собирается дневная статистика;
- формируется Excel-отчёт;
- отчёт может отправиться в Telegram;
...
- получения дневного отчёта;
- получения логов;
- получения backup-файлов;
- уведомлений о критичных ошибках.

Нижнее меню Telegram:

- `Дневной отчёт` - показать итоги дня;
- `Статус backend` - проверить, жив ли backend;
- `История импортов` - показать последние импорты;
- `Помощь` - показать краткую инструкцию.

Системная кнопка меню команд Telegram рядом с полем ввода открывает те же действия через команды `/report`, `/health`, `/imports`, `/help`.
...
10. Напечатай сводку.
11. В конце смены нажми `ЗАВЕРШИТЬ ДЕНЬ (ОТЧЁТ)`.
