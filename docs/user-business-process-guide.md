...
1. Если Smartup/Excel содержит дату отгрузки в колонке, имени файла или строках над шапкой, эта дата является главным источником. Telegram-дата используется только когда в Excel нет надёжной даты. Если Telegram-дата конфликтует с Excel-даты, импорт останавливается до создания заказов и SkladBot-заявок, а бот показывает кнопки `Использовать дату Excel` и `Отменить импорт`.
