...
- Windows candidate `2.0.48` не считать опубликованным до final preflight; переключение public channel на exact `2.0.48`/`onefile_exe` выполняется только единым immutable promotion.
...
Candidate preflight проверяет локальный `2.0.48`, текущий поддерживаемый public channel и отсутствие tracked runtime/secret-файлов; он не доказывает публикацию. Final preflight отдельно требует exact опубликованный `2.0.48`, `onefile_exe`, downloads и attestations.
