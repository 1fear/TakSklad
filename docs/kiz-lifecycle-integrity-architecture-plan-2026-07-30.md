...
### `[R2]` Разбор заказа: установленная хронология

Заказ `WH-R-206034`, получатель по Перечислению, шесть маркируемых позиций по одному
блоку. Полная цепочка шести КИЗов по ledger и audit:

| Когда, UTC | Что | Источник и актор | Доказательство |
|---|---|---|---|
| 09.07 13:00-13:01 | `outbound` шести кодов в первый заказ того же получателя | backend | ledger |
| 10.07 04:22 | полный `return` всех шести | backend, рабочая станция оператора | ledger |
| 10.07 13:39 | `re_outbound` всех шести во второй заказ того же получателя | `google_sheets`, актор пуст | ledger, плюс шесть audit `google_sheets_backend_sync_conflict` в ту же секунду |
| 14.07 09:10 | заказ закрыт как `completed_without_kiz` с причиной «фактически отпикан и отгружен, закрытие по прямому указанию» | web, актор оператора-владельца | audit |
| между 14.07 и 24.07 | статус вернулся в `not_completed` | **нет записи** | пробел |
| 24.07 11:05 и 11:21 | шесть `undo` через API, строки скана удалены | backend, актор оператора-владельца | шесть audit `scan_code_deleted` |
| 24.07 11:11 | первый освобождённый код выдан другому клиенту по Терминалу | backend | ledger |
| 27.07 14:26 | этот код возвращён тем клиентом | backend | ledger |
| 29.07 07:03, 09:10, 11:17 | ещё три кода выданы трём другим клиентам, один из них по Перечислению | backend | ledger |

Итог по кодам: два свободны и могут быть отсканированы в любой момент, четыре
числятся за четырьмя другими юридическими лицами, один из четырёх по Перечислению,
то есть попал в клиентский документ другого получателя.

Что доказано про пробел в середине:

- `undo` 24 июля прошли именно через API: на каждый есть audit `scan_code_deleted`
  с актором;
- задеплоенный на тот момент образ собран из коммита `a3462a7` от 22 июля, и в нём
  guard присутствует: `undo` отклоняется для неактивного заказа
  (`orders_service.py:357` в этом коммите), а `completed_without_kiz` ставит
  `completed` (`order_actions_service.py:174` в этом коммите);
- следовательно на 24 июля заказ уже был `not_completed`, иначе API вернул бы 409;
- ни `reset`, ни `restore` по заказу нет ни в ledger, ни в audit;
- Google Sheets runtime выведен миграцией `20260716_0019`, а cutover-репаратор 16 июля
  трогал шесть сканов шести других заказов, не этот

Вывод: переход `completed` в `not_completed` выполнен прямой записью в базу без
audit, в окне с 14 по 24 июля. Роль приложения superuser (6.26), поэтому технически
это доступно любому, у кого есть та же креда. Штатного пути не существовало ни тогда,
ни сейчас.

Чего система не доказывает и без операторской сверки доказать не может: получил ли
первый получатель товар физически. Операторская причина закрытия утверждает, что да.
Если это так, четыре кода физически ушли с первым получателем, а системно закреплены
за другими. Если нет, у четырёх других получателей коды корректны, а закрытие 14 июля
было ошибочным. Разрешает это только физическая сверка, а не БД
