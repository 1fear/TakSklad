...
`[R2]` Требование «нет legal entity, нет seal» в первом релизе останавливает
клиентскую доставку: `tax_id` и стабильный идентификатор контрагента отсутствуют в
модели данных полностью, authoritative source не выбран (§19.1), поэтому seal не
прошёл бы ни один Transfer-заказ, XLSX не был бы построен и файл клиенту не ушёл бы.
Правило вводится ступенями.
...
4. зафиксировать SQL-определения «активный КИЗ», «активный скан», «текущий
   владелец», собрать read-only query pack инвариантов §15
5. снять distinct-значения `payment_type` в production и разметить их
6. составить writer registry, включая `tools/*`, и назначить владельца каждому пути
7. измерить фактическую долю Transfer-заказов без разрешимого юридического
   получателя, это проверка допущения из 7.4
