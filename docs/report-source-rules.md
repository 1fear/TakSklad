...
- `Товары заявок` выводит одну строку на код маркировки: все текущие активные `ScanCode`, однозначно связанные с заявками, вошедшими в SkladBot daily, независимо от даты скана и типа оплаты. Код привязывается к строке товара по product key самого кода, код без совпавшего товара всё равно выводится отдельной строкой заявки. Колонка `Тип оплаты` на этом листе читается из комментария заявки тем же правилом, что и на листе `Заявки`, отдельного источника у неё нет. Времени скана в клиентском XLSX нет. Колонка `КИЗов` на листе `Заявки` равна `0` только при доказанной связи и отсутствии активных кодов; при неоднозначности она пустая. Несвязанные терминальные сканы не выводятся в клиентский XLSX.
...
- In-scope строки попадают в `Заявки` независимо от статуса заявки. Колонка `Статус` показывает `Выполнена` или `Не выполнена`, незавершенная заявка не выбрасывается в diagnostics и сама по себе не делает coverage `partial`. Статус остается видимым во внутренней `date_diagnostics` через `completed_only`, `archived_only`, `neither`.
