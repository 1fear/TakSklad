...
| Ежедневный SkladBot отчет | `/skladbot_daily ДД.ММ.ГГГГ`, schedule `22:00` | SkladBot API: requests/detail, transactions, products/stock | Не используется | Фактическая приемка берется из `acceptedAmount`, если SkladBot его отдал. Значение уже в блоках, делить на 10 нельзя. |
