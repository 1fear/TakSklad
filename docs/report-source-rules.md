...
- `Коды маркировок` содержит все текущие активные `ScanCode`, однозначно связанные с заявками, вошедшими в SkladBot daily, независимо от даты скана и типа оплаты. Колонка `КИЗов` равна `0` только при доказанной связи и отсутствии активных кодов; при неоднозначности она пустая. Несвязанные терминальные сканы не выводятся в клиентский XLSX.
