...
Назначение: ручная проверка Telegram import и Windows desktop acceptance после публикации 2.0.13 manifest. Обновления через `version.json` разрешены; текущая линия 2.0.13 переведена в forced rollout.
...
- Не публиковать новый Windows release поверх 2.0.13 без повторной проверки.
...
- [ ] `version.json` указывает на `2.0.13`, `mandatory=true`, ссылки и SHA заполнены.
