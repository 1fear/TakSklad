...
SKLADBOT_COVERAGE_ATTEMPTS="${ACCEPTANCE_SKLADBOT_COVERAGE_ATTEMPTS:-4}"
SKLADBOT_COVERAGE_RETRY_DELAY_SECONDS="${ACCEPTANCE_SKLADBOT_COVERAGE_RETRY_DELAY_SECONDS:-20}"

if [[ ! "$SKLADBOT_COVERAGE_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
  echo "ACCEPTANCE_SKLADBOT_COVERAGE_ATTEMPTS must be a positive integer" >&2
  exit 2
fi
if [[ ! "$SKLADBOT_COVERAGE_RETRY_DELAY_SECONDS" =~ ^[0-9]+$ ]]; then
  echo "ACCEPTANCE_SKLADBOT_COVERAGE_RETRY_DELAY_SECONDS must be a non-negative integer" >&2
  exit 2
fi
...
smartup_automation_status = int(sys.argv[18])
smartup_automation_output = sys.argv[19].strip()
...
forced_rollout = (
    bool(version_info.get("latest_version"))
    and version_info.get("latest_version") == version_info.get("min_supported_version")
    and version_info.get("mandatory") is True
)
if not paused_rollout and not forced_rollout:
    errors.append("version.json must be a paused 1.1.7 rollout or a structurally complete forced rollout")
if forced_rollout:
