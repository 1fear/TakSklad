...
forced_rollout = (
    version_info.get("latest_version") == "2.0.33"
    and version_info.get("min_supported_version") == "2.0.33"
    and version_info.get("mandatory") is True
)
if not paused_rollout and not forced_rollout:
    errors.append("version.json must be paused 1.1.7 rollout or forced 2.0.33 rollout")
if forced_rollout:
