...
Ежедневный SkladBot отчет собирается read-only из SkladBot requests/detail, warehouse movements и stock endpoints. Primary scope: `Дата выгрузки` / `unloading_date` или movement date за дату отчета; `created_at`, `completed_at`, `archived_at` остаются diagnostics и сами не включают строку в operational totals.

Заявки, созданные сегодня на будущую дату выгрузки, не входят в `Заявки`/`Товары заявок` и не меняют операционные итоги дня. Они выводятся отдельным листом `Будущие отгрузки`, counters `future_unloading_requests`/`future_unloading_blocks` и строкой в Telegram summary.
